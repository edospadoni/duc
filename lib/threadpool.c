/*
 * Stolen from https://docs.oracle.com/cd/E19253-01/816-5137/ggedd/index.html
 *
 * I'm not sure about the license of this code, so we should either find out
 * what the license is, or write our own implementation of a thread pool.
 *
 * Thread pool implementation.  See <thr_pool.h> for interface declarations.
 */
...
static void cleanup_mutex(void *ptr)
{
	pthread_mutex_t *m = ptr;
...
{
	(void) pthread_mutex_lock(&pool->pool_mutex);
	pthread_cleanup_push(cleanup_mutex, &pool->pool_mutex);
